...
static const char* colnamebuf[] = {"@_c0", "@_c1", "@_c2", "@_c3"};
...
static SQLWCHAR* uc_strchr(SQLWCHAR* str, SQLWCHAR c)
{
    if (str != NULL)
    {
        while ((*str != '\0') && (*str != c))
...
static int mkutf(char* dest, SQLWCHAR* src, int len)
{
    int i;
    char* cp = dest;
    ...
    for (i = 0; i < len; i++)
    {
        ...
        {
            if ((sizeof(SQLWCHAR) == (2 * sizeof(char))) && (c >= 0xd800) &&
                (c <= 0xdbff) && ((i + 1) < len))
            {
...
static VALUE uc_tainted_str_new(SQLWCHAR* str, int len)
{
    VALUE v;
...
static VALUE uc_tainted_str_new2(SQLWCHAR* str)
{
    return uc_tainted_str_new(str, uc_strlen(str));
}

static VALUE uc_str_new(SQLWCHAR* str, int len)
{
    VALUE v;
    char* cp = xmalloc(len * 6 + 1);
    int ulen = 0;
...
static VALUE uc_str_new2(SQLWCHAR* str)
{
    return uc_str_new(str, uc_strlen(str));
}

static VALUE uc_str_cat(VALUE v, SQLWCHAR* str, int len)
{
    VALUE vv = v;
    char* cp = xmalloc(len * 6 + 1);
    int ulen = 0;
        ...
        if (uc != NULL)
        {
            while (str < strend)
            {
                ...
                else if (c < 0xf8)
                {
                    ...
                    {
                        unsigned long t =
                            ((c & 0x03) << 18) | ((str[1] & 0x3f) << 12) |
                            ((str[2] & 0x3f) << 6) | (str[4] & 0x3f);
                ...
                else if (c < 0xfc)
                {
                    ...
                    {
                        unsigned long t =
                            ((c & 0x01) << 24) | ((str[1] & 0x3f) << 18) |
                            ((str[2] & 0x3f) << 12) | ((str[4] & 0x3f) << 6) |
                            (str[5] & 0x3f);
...
static void uc_free(SQLWCHAR* str)
{
    if (str != NULL)
    {
        xfree(str);
...
static void list_init(LINK* link, int offs)
{
    link->succ = link->pred = link->head = NULL;
    link->offs = offs;
}

static void list_add(LINK* link, LINK* head)
{
    if (link->head != NULL)
    {
...
static int list_empty(LINK* head) { return head->succ == NULL; }
...
static void free_env(ENV* e)
{
    ...
    {
        callsql(SQL_NULL_HENV, e->henv, SQL_NULL_HSTMT, SQLFreeEnv(e->henv),
                "SQLFreeEnv");
...
static void link_dbc(DBC* p, ENV* e)
{
    p->envp = e;
...
static void free_stmt_sub(STMT* q, int withp)
{
    int i;
...
static void link_stmt(STMT* q, DBC* p)
{
    q->dbcp = p;
...
static void free_stmt(STMT* q)
{
    ...
    if (q->hstmt != SQL_NULL_HSTMT)
    {
        ...
        fprintf(stderr,
                "WARNING: #<ODBC::Statement:0x%lx> was not dropped"
...
static char* set_err(const char* msg, int warn)
{
...
static char* get_err_or_info(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt,
                             int isinfo)
{
#ifdef UNICODE
    ...
    while (!done)
    {
        ...
        switch (err)
        {
            ...
            case SQL_ERROR:
                v = rb_str_new2(
                    "INTERN (0) [RubyODBC]Error reading error message");
                done = 1;
                break;
            default:
...
#if defined(HAVE_SQLINSTALLERERROR) ||                                         \
    (defined(UNICODE) && defined(HAVE_SQLINSTALLERERRORW))
static char* get_installer_err()
{
#ifdef UNICODE
...
static char* get_err(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt)
{
...
static int succeeded_common(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt,
                            SQLRETURN ret, char** msgp)
{
...
static VALUE dbc_raise(VALUE self, VALUE msg)
{
    VALUE a, v;
...
static VALUE dbc_dsns(VALUE self)
{
#ifdef UNICODE
    ...
    aret = rb_ary_new();
    while (succeeded(
        e->henv, SQL_NULL_HDBC, SQL_NULL_HSTMT,
        SQLDataSources(e->henv,
                       (SQLUSMALLINT)(first ? SQL_FETCH_FIRST : SQL_FETCH_NEXT),
                       (SQLTCHAR*)dsn, (SQLSMALLINT)sizeof(dsn), &dsnLen,
                       (SQLTCHAR*)descr, (SQLSMALLINT)sizeof(descr), &descrLen),
        NULL, "SQLDataSources"))
    {
        ...
        descrLen = (descrLen == 0) ? (SQLSMALLINT)uc_strlen(descr)
...
static VALUE dbc_drivers(VALUE self)
{
#ifdef UNICODE
    ...
    while (succeeded(
        ...
        SQLDrivers(e->henv,
                   (SQLUSMALLINT)(first ? SQL_FETCH_FIRST : SQL_FETCH_NEXT),
                   (SQLTCHAR*)driver, (SQLSMALLINT)sizeof(driver), &driverLen,
                   (SQLTCHAR*)attrs, (SQLSMALLINT)sizeof(attrs), &attrsLen),
        NULL, "SQLDrivers"))
    {
        ...
        for (attr = attrs; *attr; attr += uc_strlen(attr) + 1)
        {
            ...
            {
                rb_hash_aset(
                    h, uc_tainted_str_new(attr, (p - attr) / sizeof(SQLWCHAR)),
        ...
        for (attr = attrs; *attr; attr += strlen(attr) + 1)
        {
            ...
            if ((p != NULL) && (p != attr))
            {
                rb_hash_aset(h, rb_str_new(attr, p - attr), rb_str_new2(p + 1));
                count++;
            }
        }
#endif
...
#ifdef HAVE_ODBCINST_H
static VALUE conf_dsn(int argc, VALUE* argv, VALUE self, int op)
{
    ...
    if (have_w)
    {
        ...
        if (SQLConfigDataSourceW(NULL, (WORD)op, (LPWSTR)sdrv, (LPWSTR)sastr))
        {
            uc_free(sdrv);
            uc_free(sastr);
            return Qnil;
    ...
    else
    {
        ...
        if (SQLConfigDataSource(NULL, (WORD)op, (LPCSTR)sdrv, (LPCSTR)sastr))
        {
            return Qnil;
        }
    }
...
static VALUE dbc_adddsn(int argc, VALUE* argv, VALUE self)
{
#ifdef HAVE_ODBCINST_H
...
static VALUE dbc_confdsn(int argc, VALUE* argv, VALUE self)
{
#ifdef HAVE_ODBCINST_H
...
static VALUE dbc_deldsn(int argc, VALUE* argv, VALUE self)
{
#ifdef HAVE_ODBCINST_H
...
static VALUE dbc_wfdsn(int argc, VALUE* argv, VALUE self)
{
#ifdef HAVE_ODBCINST_H
    ...
    else
    {
        ...
        if (SQLWriteFileDSN((LPCSTR)sfname, (LPCSTR)saname, (LPCSTR)skname,
                            (LPCSTR)sval))
        {
            return Qnil;
        }
...
static VALUE dbc_rfdsn(int argc, VALUE* argv, VALUE self)
{
#ifdef HAVE_ODBCINST_H
    ...
    if (have_w)
    {
        ...
        rc = SQLReadFileDSNW(sfname, saname, skname, valbuf, sizeof(valbuf),
                             NULL);
        uc_free(sfname);
        uc_free(saname);
        uc_free(skname);
    ...
    else
    {
        ...
        skname = (SQLWCHAR*)StringValueCStr(kname);
        valbuf[0] = '\0';
        if (SQLReadFileDSN((LPCSTR)sfname, (LPCSTR)saname, (LPCSTR)skname,
                           (LPSTR)valbuf, sizeof(valbuf), NULL))
        {
    ...
    skname = StringValueCStr(kname);
    valbuf[0] = '\0';
    if (SQLReadFileDSN(sfname, saname, skname, valbuf, sizeof(valbuf), NULL))
    {
...
static VALUE dbc_error(VALUE self) { return rb_cvar_get(Cobj, IDataterror); }

static VALUE dbc_warn(VALUE self) { return rb_cvar_get(Cobj, IDatatinfo); }
...
static VALUE dbc_connect(int argc, VALUE* argv, VALUE self)
{
    ENV* e;
    DBC* p;
                   ...
                   &msg, "SQLConnect('%s')", sdsn))
    {
        ...
        uc_free(spasswd);
#endif
        callsql(SQL_NULL_HENV, dbc, SQL_NULL_HSTMT, SQLFreeConnect(dbc),
                "SQLFreeConnect");
        rb_raise(Cerror, "%s", msg);
    }
#ifdef UNICODE
...
static VALUE dbc_drvconnect(VALUE self, VALUE drv)
{
    ENV* e;
    DBC* p;
#ifdef UNICODE
...
static VALUE dbc_timefmt(int argc, VALUE* argv, VALUE self)
{
    DBC* p = get_dbc(self);
    VALUE val;
...
static VALUE dbc_timeutc(int argc, VALUE* argv, VALUE self)
{
    DBC* p = get_dbc(self);
    VALUE val;
...
static VALUE dbc_use_scn(int argc, VALUE* argv, VALUE self)
{
    DBC* p = get_dbc(self);
    VALUE val;
...
static VALUE dbc_disconnect(int argc, VALUE* argv, VALUE self)
{
    DBC* p = get_dbc(self);
    VALUE nodrop = Qfalse;
    char* msg;
...
#define GI_CONST_SINT(x)                                                       \
    {                                                                          \
#x, x, SQL_C_SHORT                                                     \
    }
#define GI_CONST_INT(x)                                                        \
    {                                                                          \
#x, x, SQL_C_LONG                                                      \
    }
#define GI_CONST_BITS(x)                                                       \
    {                                                                          \
#x, x, SQL_C_LONG                                                      \
    }
#define GI_CONST_STR(x)                                                        \
    {                                                                          \
#x, x, SQL_C_CHAR                                                      \
    }
#define GI_CONST_END                                                           \
...
static struct
{
    ...
    GI_CONST_SINT(SQL_ACTIVE_CONNECTIONS), GI_CONST_SINT(SQL_ACTIVE_STATEMENTS),
    GI_CONST_INT(SQL_ASYNC_MODE), GI_CONST_SINT(SQL_CATALOG_LOCATION),
    ...
    GI_CONST_INT(SQL_CURSOR_SENSITIVITY), GI_CONST_INT(SQL_DDL_INDEX),
    GI_CONST_INT(SQL_DEFAULT_TXN_ISOLATION), GI_CONST_INT(SQL_DRIVER_HDBC),
    GI_CONST_INT(SQL_DRIVER_HENV), GI_CONST_INT(SQL_DRIVER_HDESC),
    GI_CONST_INT(SQL_DRIVER_HLIB), GI_CONST_INT(SQL_DRIVER_HSTMT),
    GI_CONST_SINT(SQL_FILE_USAGE), GI_CONST_SINT(SQL_GROUP_BY),
    ...
    GI_CONST_SINT(SQL_MAX_IDENTIFIER_LEN), GI_CONST_INT(SQL_MAX_INDEX_SIZE),
    GI_CONST_SINT(SQL_MAX_OWNER_NAME_LEN),
    GI_CONST_SINT(SQL_MAX_PROCEDURE_NAME_LEN),
    GI_CONST_SINT(SQL_MAX_QUALIFIER_NAME_LEN), GI_CONST_INT(SQL_MAX_ROW_SIZE),
    GI_CONST_SINT(SQL_MAX_SCHEMA_NAME_LEN), GI_CONST_INT(SQL_MAX_STATEMENT_LEN),
    GI_CONST_SINT(SQL_MAX_TABLE_NAME_LEN),
    GI_CONST_SINT(SQL_MAX_TABLES_IN_SELECT),
    GI_CONST_SINT(SQL_MAX_USER_NAME_LEN),
    GI_CONST_SINT(SQL_NON_NULLABLE_COLUMNS), GI_CONST_SINT(SQL_NULL_COLLATION),
    ...
    GI_CONST_INT(SQL_SQL_CONFORMANCE), GI_CONST_SINT(SQL_TXN_CAPABLE),

    /* yielding ints (but bitmasks) */
    GI_CONST_BITS(SQL_AGGREGATE_FUNCTIONS), GI_CONST_BITS(SQL_ALTER_DOMAIN),
    GI_CONST_BITS(SQL_ALTER_TABLE), GI_CONST_BITS(SQL_BATCH_ROW_COUNT),
    GI_CONST_BITS(SQL_BATCH_SUPPORT), GI_CONST_BITS(SQL_BOOKMARK_PERSISTENCE),
    GI_CONST_BITS(SQL_CATALOG_USAGE), GI_CONST_BITS(SQL_CONVERT_BINARY),
    GI_CONST_BITS(SQL_CONVERT_BIT), GI_CONST_BITS(SQL_CONVERT_CHAR),
#ifdef SQL_CONVERT_GUID
    GI_CONST_BITS(SQL_CONVERT_GUID),
#endif
    GI_CONST_BITS(SQL_CONVERT_DATE), GI_CONST_BITS(SQL_CONVERT_DECIMAL),
    GI_CONST_BITS(SQL_CONVERT_DOUBLE), GI_CONST_BITS(SQL_CONVERT_FLOAT),
    GI_CONST_BITS(SQL_CONVERT_FUNCTIONS), GI_CONST_BITS(SQL_CONVERT_INTEGER),
    ...
    GI_CONST_BITS(SQL_CONVERT_LONGVARCHAR), GI_CONST_BITS(SQL_CONVERT_NUMERIC),
    GI_CONST_BITS(SQL_CONVERT_REAL), GI_CONST_BITS(SQL_CONVERT_SMALLINT),
    GI_CONST_BITS(SQL_CONVERT_TIME), GI_CONST_BITS(SQL_CONVERT_TIMESTAMP),
    GI_CONST_BITS(SQL_CONVERT_TINYINT), GI_CONST_BITS(SQL_CONVERT_VARBINARY),
    GI_CONST_BITS(SQL_CONVERT_VARCHAR), GI_CONST_BITS(SQL_CONVERT_WCHAR),
    GI_CONST_BITS(SQL_CONVERT_WLONGVARCHAR),
    GI_CONST_BITS(SQL_CONVERT_WVARCHAR), GI_CONST_BITS(SQL_CREATE_ASSERTION),
    GI_CONST_BITS(SQL_CREATE_CHARACTER_SET),
    GI_CONST_BITS(SQL_CREATE_COLLATION), GI_CONST_BITS(SQL_CREATE_DOMAIN),
    GI_CONST_BITS(SQL_CREATE_SCHEMA), GI_CONST_BITS(SQL_CREATE_TABLE),
    GI_CONST_BITS(SQL_CREATE_TRANSLATION), GI_CONST_BITS(SQL_CREATE_VIEW),
    GI_CONST_BITS(SQL_DATETIME_LITERALS), GI_CONST_BITS(SQL_DROP_ASSERTION),
    GI_CONST_BITS(SQL_DROP_CHARACTER_SET), GI_CONST_BITS(SQL_DROP_COLLATION),
    GI_CONST_BITS(SQL_DROP_DOMAIN), GI_CONST_BITS(SQL_DROP_SCHEMA),
    GI_CONST_BITS(SQL_DROP_TABLE), GI_CONST_BITS(SQL_DROP_TRANSLATION),
    GI_CONST_BITS(SQL_DROP_VIEW), GI_CONST_BITS(SQL_DTC_TRANSITION_COST),
    ...
    GI_CONST_BITS(SQL_INDEX_KEYWORDS), GI_CONST_BITS(SQL_INFO_SCHEMA_VIEWS),
    GI_CONST_BITS(SQL_INSERT_STATEMENT), GI_CONST_BITS(SQL_LOCK_TYPES),
    GI_CONST_BITS(SQL_NUMERIC_FUNCTIONS), GI_CONST_BITS(SQL_OJ_CAPABILITIES),
    GI_CONST_BITS(SQL_OWNER_USAGE), GI_CONST_BITS(SQL_POS_OPERATIONS),
    GI_CONST_BITS(SQL_POSITIONED_STATEMENTS),
    GI_CONST_BITS(SQL_QUALIFIER_USAGE), GI_CONST_BITS(SQL_SCHEMA_USAGE),
    GI_CONST_BITS(SQL_SCROLL_CONCURRENCY), GI_CONST_BITS(SQL_SCROLL_OPTIONS),
    ...
    GI_CONST_BITS(SQL_STATIC_SENSITIVITY), GI_CONST_BITS(SQL_STRING_FUNCTIONS),
    GI_CONST_BITS(SQL_SUBQUERIES), GI_CONST_BITS(SQL_SYSTEM_FUNCTIONS),
    GI_CONST_BITS(SQL_TIMEDATE_ADD_INTERVALS),
    GI_CONST_BITS(SQL_TIMEDATE_DIFF_INTERVALS),
    GI_CONST_BITS(SQL_TIMEDATE_FUNCTIONS),
    GI_CONST_BITS(SQL_TXN_ISOLATION_OPTION), GI_CONST_BITS(SQL_UNION),

    /* yielding strings */
    GI_CONST_STR(SQL_ACCESSIBLE_PROCEDURES),
    GI_CONST_STR(SQL_ACCESSIBLE_TABLES), GI_CONST_STR(SQL_CATALOG_NAME),
    GI_CONST_STR(SQL_CATALOG_NAME_SEPARATOR), GI_CONST_STR(SQL_CATALOG_TERM),
    GI_CONST_STR(SQL_COLLATION_SEQ), GI_CONST_STR(SQL_COLUMN_ALIAS),
    GI_CONST_STR(SQL_DATA_SOURCE_NAME), GI_CONST_STR(SQL_DATA_SOURCE_READ_ONLY),
    GI_CONST_STR(SQL_DATABASE_NAME), GI_CONST_STR(SQL_DBMS_NAME),
    GI_CONST_STR(SQL_DBMS_VER), GI_CONST_STR(SQL_DESCRIBE_PARAMETER),
    GI_CONST_STR(SQL_DM_VER), GI_CONST_STR(SQL_DRIVER_NAME),
    GI_CONST_STR(SQL_DRIVER_ODBC_VER), GI_CONST_STR(SQL_DRIVER_VER),
    GI_CONST_STR(SQL_EXPRESSIONS_IN_ORDERBY),
    GI_CONST_STR(SQL_IDENTIFIER_QUOTE_CHAR), GI_CONST_STR(SQL_INTEGRITY),
    GI_CONST_STR(SQL_KEYWORDS), GI_CONST_STR(SQL_LIKE_ESCAPE_CLAUSE),
    GI_CONST_STR(SQL_MAX_ROW_SIZE_INCLUDES_LONG),
    GI_CONST_STR(SQL_MULT_RESULT_SETS), GI_CONST_STR(SQL_MULTIPLE_ACTIVE_TXN),
    GI_CONST_STR(SQL_NEED_LONG_DATA_LEN), GI_CONST_STR(SQL_ODBC_SQL_OPT_IEF),
    GI_CONST_STR(SQL_ODBC_VER), GI_CONST_STR(SQL_ORDER_BY_COLUMNS_IN_SELECT),
    GI_CONST_STR(SQL_OUTER_JOINS), GI_CONST_STR(SQL_OWNER_TERM),
    GI_CONST_STR(SQL_PROCEDURE_TERM), GI_CONST_STR(SQL_PROCEDURES),
    GI_CONST_STR(SQL_QUALIFIER_NAME_SEPARATOR),
    GI_CONST_STR(SQL_QUALIFIER_TERM), GI_CONST_STR(SQL_ROW_UPDATES),
    GI_CONST_STR(SQL_SCHEMA_TERM), GI_CONST_STR(SQL_SEARCH_PATTERN_ESCAPE),
    GI_CONST_STR(SQL_SERVER_NAME), GI_CONST_STR(SQL_SPECIAL_CHARACTERS),
    GI_CONST_STR(SQL_TABLE_TERM), GI_CONST_STR(SQL_USER_NAME),
    GI_CONST_STR(SQL_XOPEN_CLI_YEAR),

    /* end of table */
    GI_CONST_END};

#define GI_CONST_BITMAP(x)                                                     \
    {                                                                          \
#x, x                                                                  \
    }
#define GI_CONST_BITMAP_END                                                    \
    {                                                                          \
        NULL, 0                                                                \
    }
static struct
{
    ...
    GI_CONST_BITMAP(SQL_AD_DROP_DOMAIN_DEFAULT), GI_CONST_BITMAP(SQL_AF_ALL),
    GI_CONST_BITMAP(SQL_AF_AVG), GI_CONST_BITMAP(SQL_AF_COUNT),
    GI_CONST_BITMAP(SQL_AF_DISTINCT), GI_CONST_BITMAP(SQL_AF_MAX),
    GI_CONST_BITMAP(SQL_AF_MIN), GI_CONST_BITMAP(SQL_AF_SUM),
    GI_CONST_BITMAP(SQL_AM_CONNECTION), GI_CONST_BITMAP(SQL_AM_NONE),
    GI_CONST_BITMAP(SQL_AM_STATEMENT), GI_CONST_BITMAP(SQL_AT_ADD_COLUMN),
    ...
    GI_CONST_BITMAP(SQL_AT_SET_COLUMN_DEFAULT), GI_CONST_BITMAP(SQL_BP_CLOSE),
    GI_CONST_BITMAP(SQL_BP_DELETE), GI_CONST_BITMAP(SQL_BP_DROP),
    GI_CONST_BITMAP(SQL_BP_OTHER_HSTMT), GI_CONST_BITMAP(SQL_BP_SCROLL),
    GI_CONST_BITMAP(SQL_BP_TRANSACTION), GI_CONST_BITMAP(SQL_BP_UPDATE),
    GI_CONST_BITMAP(SQL_BRC_EXPLICIT), GI_CONST_BITMAP(SQL_BRC_PROCEDURES),
    ...
    GI_CONST_BITMAP(SQL_BS_SELECT_PROC), GI_CONST_BITMAP(SQL_CA1_ABSOLUTE),
    GI_CONST_BITMAP(SQL_CA1_BOOKMARK), GI_CONST_BITMAP(SQL_CA1_BULK_ADD),
    ...
    GI_CONST_BITMAP(SQL_CA1_LOCK_UNLOCK), GI_CONST_BITMAP(SQL_CA1_NEXT),
    GI_CONST_BITMAP(SQL_CA1_POS_DELETE),
    GI_CONST_BITMAP(SQL_CA1_POSITIONED_DELETE),
    GI_CONST_BITMAP(SQL_CA1_POSITIONED_UPDATE),
    GI_CONST_BITMAP(SQL_CA1_POS_POSITION), GI_CONST_BITMAP(SQL_CA1_POS_REFRESH),
    GI_CONST_BITMAP(SQL_CA1_POS_UPDATE), GI_CONST_BITMAP(SQL_CA1_RELATIVE),
    ...
    GI_CONST_BITMAP(SQL_CA_CREATE_ASSERTION), GI_CONST_BITMAP(SQL_CB_CLOSE),
    GI_CONST_BITMAP(SQL_CB_DELETE), GI_CONST_BITMAP(SQL_CB_NON_NULL),
    GI_CONST_BITMAP(SQL_CB_NULL), GI_CONST_BITMAP(SQL_CB_PRESERVE),
    GI_CONST_BITMAP(SQL_CC_CLOSE), GI_CONST_BITMAP(SQL_CC_DELETE),
    GI_CONST_BITMAP(SQL_CCOL_CREATE_COLLATION),
    GI_CONST_BITMAP(SQL_CC_PRESERVE), GI_CONST_BITMAP(SQL_CCS_COLLATE_CLAUSE),
    GI_CONST_BITMAP(SQL_CCS_CREATE_CHARACTER_SET),
    GI_CONST_BITMAP(SQL_CCS_LIMITED_COLLATION),
    GI_CONST_BITMAP(SQL_CDO_COLLATION), GI_CONST_BITMAP(SQL_CDO_CONSTRAINT),
    ...
    GI_CONST_BITMAP(SQL_CDO_CREATE_DOMAIN), GI_CONST_BITMAP(SQL_CDO_DEFAULT),
    GI_CONST_BITMAP(SQL_CL_END), GI_CONST_BITMAP(SQL_CL_START),
    GI_CONST_BITMAP(SQL_CN_ANY), GI_CONST_BITMAP(SQL_CN_DIFFERENT),
    GI_CONST_BITMAP(SQL_CN_NONE), GI_CONST_BITMAP(SQL_CONCUR_TIMESTAMP),
    GI_CONST_BITMAP(SQL_CR_CLOSE), GI_CONST_BITMAP(SQL_CR_DELETE),
    GI_CONST_BITMAP(SQL_CR_PRESERVE), GI_CONST_BITMAP(SQL_CS_AUTHORIZATION),
    ...
    GI_CONST_BITMAP(SQL_CU_TABLE_DEFINITION), GI_CONST_BITMAP(SQL_CV_CASCADED),
    GI_CONST_BITMAP(SQL_CV_CHECK_OPTION), GI_CONST_BITMAP(SQL_CV_CREATE_VIEW),
    GI_CONST_BITMAP(SQL_CV_LOCAL), GI_CONST_BITMAP(SQL_CVT_BIGINT),
    GI_CONST_BITMAP(SQL_CVT_BINARY), GI_CONST_BITMAP(SQL_CVT_BIT),
    GI_CONST_BITMAP(SQL_CVT_CHAR), GI_CONST_BITMAP(SQL_CVT_DATE),
    GI_CONST_BITMAP(SQL_CVT_DECIMAL), GI_CONST_BITMAP(SQL_CVT_DOUBLE),
    GI_CONST_BITMAP(SQL_CVT_FLOAT), GI_CONST_BITMAP(SQL_CVT_INTEGER),
    ...
    GI_CONST_BITMAP(SQL_CVT_LONGVARCHAR), GI_CONST_BITMAP(SQL_CVT_NUMERIC),
    GI_CONST_BITMAP(SQL_CVT_REAL), GI_CONST_BITMAP(SQL_CVT_SMALLINT),
    GI_CONST_BITMAP(SQL_CVT_TIME), GI_CONST_BITMAP(SQL_CVT_TIMESTAMP),
    GI_CONST_BITMAP(SQL_CVT_TINYINT), GI_CONST_BITMAP(SQL_CVT_VARBINARY),
    GI_CONST_BITMAP(SQL_CVT_VARCHAR), GI_CONST_BITMAP(SQL_CVT_WCHAR),
    GI_CONST_BITMAP(SQL_CVT_WLONGVARCHAR), GI_CONST_BITMAP(SQL_CVT_WVARCHAR),
    GI_CONST_BITMAP(SQL_DA_DROP_ASSERTION),
    GI_CONST_BITMAP(SQL_DC_DROP_COLLATION),
    GI_CONST_BITMAP(SQL_DCS_DROP_CHARACTER_SET),
    GI_CONST_BITMAP(SQL_DD_CASCADE), GI_CONST_BITMAP(SQL_DD_DROP_DOMAIN),
    GI_CONST_BITMAP(SQL_DD_RESTRICT), GI_CONST_BITMAP(SQL_DI_CREATE_INDEX),
    GI_CONST_BITMAP(SQL_DI_DROP_INDEX), GI_CONST_BITMAP(SQL_DL_SQL92_DATE),
    ...
    GI_CONST_BITMAP(SQL_DL_SQL92_TIME), GI_CONST_BITMAP(SQL_DL_SQL92_TIMESTAMP),
    GI_CONST_BITMAP(SQL_DS_CASCADE), GI_CONST_BITMAP(SQL_DS_DROP_SCHEMA),
    GI_CONST_BITMAP(SQL_DS_RESTRICT), GI_CONST_BITMAP(SQL_DT_CASCADE),
    GI_CONST_BITMAP(SQL_DTC_ENLIST_EXPENSIVE),
    GI_CONST_BITMAP(SQL_DTC_UNENLIST_EXPENSIVE),
    GI_CONST_BITMAP(SQL_DT_DROP_TABLE),
    GI_CONST_BITMAP(SQL_DTR_DROP_TRANSLATION), GI_CONST_BITMAP(SQL_DT_RESTRICT),
    GI_CONST_BITMAP(SQL_DV_CASCADE), GI_CONST_BITMAP(SQL_DV_DROP_VIEW),
    GI_CONST_BITMAP(SQL_DV_RESTRICT), GI_CONST_BITMAP(SQL_FD_FETCH_ABSOLUTE),
    GI_CONST_BITMAP(SQL_FD_FETCH_BOOKMARK), GI_CONST_BITMAP(SQL_FD_FETCH_FIRST),
    GI_CONST_BITMAP(SQL_FD_FETCH_LAST), GI_CONST_BITMAP(SQL_FD_FETCH_NEXT),
    GI_CONST_BITMAP(SQL_FD_FETCH_PRIOR), GI_CONST_BITMAP(SQL_FD_FETCH_RELATIVE),
#ifdef SQL_FD_FETCH_RESUME
    GI_CONST_BITMAP(SQL_FD_FETCH_RESUME),
#endif
    GI_CONST_BITMAP(SQL_FILE_CATALOG), GI_CONST_BITMAP(SQL_FILE_NOT_SUPPORTED),
    GI_CONST_BITMAP(SQL_FILE_QUALIFIER), GI_CONST_BITMAP(SQL_FILE_TABLE),
    GI_CONST_BITMAP(SQL_FN_CVT_CAST), GI_CONST_BITMAP(SQL_FN_CVT_CONVERT),
    GI_CONST_BITMAP(SQL_FN_NUM_ABS), GI_CONST_BITMAP(SQL_FN_NUM_ACOS),
    GI_CONST_BITMAP(SQL_FN_NUM_ASIN), GI_CONST_BITMAP(SQL_FN_NUM_ATAN),
    GI_CONST_BITMAP(SQL_FN_NUM_ATAN2), GI_CONST_BITMAP(SQL_FN_NUM_CEILING),
    GI_CONST_BITMAP(SQL_FN_NUM_COS), GI_CONST_BITMAP(SQL_FN_NUM_COT),
    GI_CONST_BITMAP(SQL_FN_NUM_DEGREES), GI_CONST_BITMAP(SQL_FN_NUM_EXP),
    GI_CONST_BITMAP(SQL_FN_NUM_FLOOR), GI_CONST_BITMAP(SQL_FN_NUM_LOG),
    GI_CONST_BITMAP(SQL_FN_NUM_LOG10), GI_CONST_BITMAP(SQL_FN_NUM_MOD),
    GI_CONST_BITMAP(SQL_FN_NUM_PI), GI_CONST_BITMAP(SQL_FN_NUM_POWER),
    GI_CONST_BITMAP(SQL_FN_NUM_RADIANS), GI_CONST_BITMAP(SQL_FN_NUM_RAND),
    GI_CONST_BITMAP(SQL_FN_NUM_ROUND), GI_CONST_BITMAP(SQL_FN_NUM_SIGN),
    GI_CONST_BITMAP(SQL_FN_NUM_SIN), GI_CONST_BITMAP(SQL_FN_NUM_SQRT),
    GI_CONST_BITMAP(SQL_FN_NUM_TAN), GI_CONST_BITMAP(SQL_FN_NUM_TRUNCATE),
    GI_CONST_BITMAP(SQL_FN_STR_ASCII), GI_CONST_BITMAP(SQL_FN_STR_BIT_LENGTH),
    GI_CONST_BITMAP(SQL_FN_STR_CHAR),
    GI_CONST_BITMAP(SQL_FN_STR_CHARACTER_LENGTH),
    GI_CONST_BITMAP(SQL_FN_STR_CHAR_LENGTH), GI_CONST_BITMAP(SQL_FN_STR_CONCAT),
    GI_CONST_BITMAP(SQL_FN_STR_DIFFERENCE), GI_CONST_BITMAP(SQL_FN_STR_INSERT),
    GI_CONST_BITMAP(SQL_FN_STR_LCASE), GI_CONST_BITMAP(SQL_FN_STR_LEFT),
    GI_CONST_BITMAP(SQL_FN_STR_LENGTH), GI_CONST_BITMAP(SQL_FN_STR_LOCATE),
    GI_CONST_BITMAP(SQL_FN_STR_LOCATE_2), GI_CONST_BITMAP(SQL_FN_STR_LTRIM),
    GI_CONST_BITMAP(SQL_FN_STR_OCTET_LENGTH),
    GI_CONST_BITMAP(SQL_FN_STR_POSITION), GI_CONST_BITMAP(SQL_FN_STR_REPEAT),
    GI_CONST_BITMAP(SQL_FN_STR_REPLACE), GI_CONST_BITMAP(SQL_FN_STR_RIGHT),
    GI_CONST_BITMAP(SQL_FN_STR_RTRIM), GI_CONST_BITMAP(SQL_FN_STR_SOUNDEX),
    GI_CONST_BITMAP(SQL_FN_STR_SPACE), GI_CONST_BITMAP(SQL_FN_STR_SUBSTRING),
    GI_CONST_BITMAP(SQL_FN_STR_UCASE), GI_CONST_BITMAP(SQL_FN_SYS_DBNAME),
    GI_CONST_BITMAP(SQL_FN_SYS_IFNULL), GI_CONST_BITMAP(SQL_FN_SYS_USERNAME),
    GI_CONST_BITMAP(SQL_FN_TD_CURDATE), GI_CONST_BITMAP(SQL_FN_TD_CURRENT_DATE),
    GI_CONST_BITMAP(SQL_FN_TD_CURRENT_TIME),
    GI_CONST_BITMAP(SQL_FN_TD_CURRENT_TIMESTAMP),
    GI_CONST_BITMAP(SQL_FN_TD_CURTIME), GI_CONST_BITMAP(SQL_FN_TD_DAYNAME),
    GI_CONST_BITMAP(SQL_FN_TD_DAYOFMONTH), GI_CONST_BITMAP(SQL_FN_TD_DAYOFWEEK),
    GI_CONST_BITMAP(SQL_FN_TD_DAYOFYEAR), GI_CONST_BITMAP(SQL_FN_TD_EXTRACT),
    GI_CONST_BITMAP(SQL_FN_TD_HOUR), GI_CONST_BITMAP(SQL_FN_TD_MINUTE),
    GI_CONST_BITMAP(SQL_FN_TD_MONTH), GI_CONST_BITMAP(SQL_FN_TD_MONTHNAME),
    GI_CONST_BITMAP(SQL_FN_TD_NOW), GI_CONST_BITMAP(SQL_FN_TD_QUARTER),
    GI_CONST_BITMAP(SQL_FN_TD_SECOND), GI_CONST_BITMAP(SQL_FN_TD_TIMESTAMPADD),
    GI_CONST_BITMAP(SQL_FN_TD_TIMESTAMPDIFF), GI_CONST_BITMAP(SQL_FN_TD_WEEK),
    GI_CONST_BITMAP(SQL_FN_TD_YEAR), GI_CONST_BITMAP(SQL_FN_TSI_DAY),
    GI_CONST_BITMAP(SQL_FN_TSI_FRAC_SECOND), GI_CONST_BITMAP(SQL_FN_TSI_HOUR),
    GI_CONST_BITMAP(SQL_FN_TSI_MINUTE), GI_CONST_BITMAP(SQL_FN_TSI_MONTH),
    GI_CONST_BITMAP(SQL_FN_TSI_QUARTER), GI_CONST_BITMAP(SQL_FN_TSI_SECOND),
    GI_CONST_BITMAP(SQL_FN_TSI_WEEK), GI_CONST_BITMAP(SQL_FN_TSI_YEAR),
    ...
    GI_CONST_BITMAP(SQL_GB_NO_RELATION), GI_CONST_BITMAP(SQL_GB_NOT_SUPPORTED),
    GI_CONST_BITMAP(SQL_GD_ANY_COLUMN), GI_CONST_BITMAP(SQL_GD_ANY_ORDER),
    GI_CONST_BITMAP(SQL_GD_BLOCK), GI_CONST_BITMAP(SQL_GD_BOUND),
    GI_CONST_BITMAP(SQL_IC_LOWER), GI_CONST_BITMAP(SQL_IC_MIXED),
    GI_CONST_BITMAP(SQL_IC_SENSITIVE), GI_CONST_BITMAP(SQL_IC_UPPER),
    GI_CONST_BITMAP(SQL_IK_ALL), GI_CONST_BITMAP(SQL_IK_ASC),
    GI_CONST_BITMAP(SQL_IK_DESC), GI_CONST_BITMAP(SQL_IK_NONE),
    GI_CONST_BITMAP(SQL_IS_INSERT_LITERALS),
    GI_CONST_BITMAP(SQL_IS_INSERT_SEARCHED),
    GI_CONST_BITMAP(SQL_IS_SELECT_INTO), GI_CONST_BITMAP(SQL_ISV_ASSERTIONS),
    ...
    GI_CONST_BITMAP(SQL_ISV_DOMAINS), GI_CONST_BITMAP(SQL_ISV_KEY_COLUMN_USAGE),
    GI_CONST_BITMAP(SQL_ISV_REFERENTIAL_CONSTRAINTS),
    GI_CONST_BITMAP(SQL_ISV_SCHEMATA), GI_CONST_BITMAP(SQL_ISV_SQL_LANGUAGES),
    GI_CONST_BITMAP(SQL_ISV_TABLE_CONSTRAINTS),
    GI_CONST_BITMAP(SQL_ISV_TABLE_PRIVILEGES), GI_CONST_BITMAP(SQL_ISV_TABLES),
    GI_CONST_BITMAP(SQL_ISV_TRANSLATIONS),
    GI_CONST_BITMAP(SQL_ISV_USAGE_PRIVILEGES),
    GI_CONST_BITMAP(SQL_ISV_VIEW_COLUMN_USAGE), GI_CONST_BITMAP(SQL_ISV_VIEWS),
    GI_CONST_BITMAP(SQL_ISV_VIEW_TABLE_USAGE),
    GI_CONST_BITMAP(SQL_LCK_EXCLUSIVE), GI_CONST_BITMAP(SQL_LCK_NO_CHANGE),
    GI_CONST_BITMAP(SQL_LCK_UNLOCK), GI_CONST_BITMAP(SQL_NC_END),
    GI_CONST_BITMAP(SQL_NC_HIGH), GI_CONST_BITMAP(SQL_NC_LOW),
    GI_CONST_BITMAP(SQL_NC_START), GI_CONST_BITMAP(SQL_NNC_NON_NULL),
    GI_CONST_BITMAP(SQL_NNC_NULL), GI_CONST_BITMAP(SQL_OAC_LEVEL1),
    GI_CONST_BITMAP(SQL_OAC_LEVEL2), GI_CONST_BITMAP(SQL_OAC_NONE),
    GI_CONST_BITMAP(SQL_OIC_CORE), GI_CONST_BITMAP(SQL_OIC_LEVEL1),
    GI_CONST_BITMAP(SQL_OIC_LEVEL2), GI_CONST_BITMAP(SQL_OJ_ALL_COMPARISON_OPS),
    GI_CONST_BITMAP(SQL_OJ_FULL), GI_CONST_BITMAP(SQL_OJ_INNER),
    GI_CONST_BITMAP(SQL_OJ_LEFT), GI_CONST_BITMAP(SQL_OJ_NESTED),
    GI_CONST_BITMAP(SQL_OJ_NOT_ORDERED), GI_CONST_BITMAP(SQL_OJ_RIGHT),
    GI_CONST_BITMAP(SQL_OSCC_COMPLIANT),
    GI_CONST_BITMAP(SQL_OSCC_NOT_COMPLIANT), GI_CONST_BITMAP(SQL_OSC_CORE),
    GI_CONST_BITMAP(SQL_OSC_EXTENDED), GI_CONST_BITMAP(SQL_OSC_MINIMUM),
    ...
    GI_CONST_BITMAP(SQL_OU_TABLE_DEFINITION), GI_CONST_BITMAP(SQL_PARC_BATCH),
    GI_CONST_BITMAP(SQL_PARC_NO_BATCH), GI_CONST_BITMAP(SQL_PAS_BATCH),
    GI_CONST_BITMAP(SQL_PAS_NO_BATCH), GI_CONST_BITMAP(SQL_PAS_NO_SELECT),
    GI_CONST_BITMAP(SQL_POS_ADD), GI_CONST_BITMAP(SQL_POS_DELETE),
    GI_CONST_BITMAP(SQL_POS_POSITION), GI_CONST_BITMAP(SQL_POS_REFRESH),
    GI_CONST_BITMAP(SQL_POS_UPDATE), GI_CONST_BITMAP(SQL_PS_POSITIONED_DELETE),
    GI_CONST_BITMAP(SQL_PS_POSITIONED_UPDATE),
    GI_CONST_BITMAP(SQL_PS_SELECT_FOR_UPDATE), GI_CONST_BITMAP(SQL_QL_END),
    GI_CONST_BITMAP(SQL_QL_START), GI_CONST_BITMAP(SQL_QU_DML_STATEMENTS),
    ...
    GI_CONST_BITMAP(SQL_SCC_ISO92_CLI), GI_CONST_BITMAP(SQL_SCCO_LOCK),
    GI_CONST_BITMAP(SQL_SCCO_OPT_ROWVER),
    GI_CONST_BITMAP(SQL_SCCO_OPT_TIMESTAMP),
    GI_CONST_BITMAP(SQL_SCCO_OPT_VALUES), GI_CONST_BITMAP(SQL_SCCO_READ_ONLY),
    GI_CONST_BITMAP(SQL_SCC_XOPEN_CLI_VERSION1),
    GI_CONST_BITMAP(SQL_SC_FIPS127_2_TRANSITIONAL),
    GI_CONST_BITMAP(SQL_SC_NON_UNIQUE), GI_CONST_BITMAP(SQL_SC_SQL92_ENTRY),
    GI_CONST_BITMAP(SQL_SC_SQL92_FULL),
    GI_CONST_BITMAP(SQL_SC_SQL92_INTERMEDIATE),
    GI_CONST_BITMAP(SQL_SC_TRY_UNIQUE), GI_CONST_BITMAP(SQL_SC_UNIQUE),
    GI_CONST_BITMAP(SQL_SDF_CURRENT_DATE),
    GI_CONST_BITMAP(SQL_SDF_CURRENT_TIME),
    GI_CONST_BITMAP(SQL_SDF_CURRENT_TIMESTAMP),
    GI_CONST_BITMAP(SQL_SFKD_CASCADE), GI_CONST_BITMAP(SQL_SFKD_NO_ACTION),
    GI_CONST_BITMAP(SQL_SFKD_SET_DEFAULT), GI_CONST_BITMAP(SQL_SFKD_SET_NULL),
    GI_CONST_BITMAP(SQL_SFKU_CASCADE), GI_CONST_BITMAP(SQL_SFKU_NO_ACTION),
    GI_CONST_BITMAP(SQL_SFKU_SET_DEFAULT), GI_CONST_BITMAP(SQL_SFKU_SET_NULL),
    GI_CONST_BITMAP(SQL_SG_DELETE_TABLE), GI_CONST_BITMAP(SQL_SG_INSERT_COLUMN),
    GI_CONST_BITMAP(SQL_SG_INSERT_TABLE),
    GI_CONST_BITMAP(SQL_SG_REFERENCES_COLUMN),
    GI_CONST_BITMAP(SQL_SG_REFERENCES_TABLE),
    GI_CONST_BITMAP(SQL_SG_SELECT_TABLE), GI_CONST_BITMAP(SQL_SG_UPDATE_COLUMN),
    ...
    GI_CONST_BITMAP(SQL_SNVF_CHAR_LENGTH), GI_CONST_BITMAP(SQL_SNVF_EXTRACT),
    GI_CONST_BITMAP(SQL_SNVF_OCTET_LENGTH), GI_CONST_BITMAP(SQL_SNVF_POSITION),
    GI_CONST_BITMAP(SQL_SO_DYNAMIC), GI_CONST_BITMAP(SQL_SO_FORWARD_ONLY),
    GI_CONST_BITMAP(SQL_SO_KEYSET_DRIVEN), GI_CONST_BITMAP(SQL_SO_MIXED),
    GI_CONST_BITMAP(SQL_SO_STATIC), GI_CONST_BITMAP(SQL_SP_BETWEEN),
    GI_CONST_BITMAP(SQL_SP_COMPARISON), GI_CONST_BITMAP(SQL_SP_EXISTS),
    GI_CONST_BITMAP(SQL_SP_IN), GI_CONST_BITMAP(SQL_SP_ISNOTNULL),
    GI_CONST_BITMAP(SQL_SP_ISNULL), GI_CONST_BITMAP(SQL_SP_LIKE),
    GI_CONST_BITMAP(SQL_SP_MATCH_FULL), GI_CONST_BITMAP(SQL_SP_MATCH_PARTIAL),
    ...
    GI_CONST_BITMAP(SQL_SP_UNIQUE), GI_CONST_BITMAP(SQL_SQ_COMPARISON),
    GI_CONST_BITMAP(SQL_SQ_CORRELATED_SUBQUERIES),
    GI_CONST_BITMAP(SQL_SQ_EXISTS), GI_CONST_BITMAP(SQL_SQ_IN),
    GI_CONST_BITMAP(SQL_SQ_QUANTIFIED), GI_CONST_BITMAP(SQL_SR_CASCADE),
    GI_CONST_BITMAP(SQL_SR_DELETE_TABLE),
    GI_CONST_BITMAP(SQL_SR_GRANT_OPTION_FOR),
    GI_CONST_BITMAP(SQL_SR_INSERT_COLUMN), GI_CONST_BITMAP(SQL_SR_INSERT_TABLE),
    GI_CONST_BITMAP(SQL_SRJO_CORRESPONDING_CLAUSE),
    GI_CONST_BITMAP(SQL_SRJO_CROSS_JOIN), GI_CONST_BITMAP(SQL_SRJO_EXCEPT_JOIN),
    ...
    GI_CONST_BITMAP(SQL_SR_REFERENCES_TABLE), GI_CONST_BITMAP(SQL_SR_RESTRICT),
    GI_CONST_BITMAP(SQL_SR_SELECT_TABLE), GI_CONST_BITMAP(SQL_SR_UPDATE_COLUMN),
    ...
    GI_CONST_BITMAP(SQL_SRVC_DEFAULT), GI_CONST_BITMAP(SQL_SRVC_NULL),
    GI_CONST_BITMAP(SQL_SRVC_ROW_SUBQUERY),
    GI_CONST_BITMAP(SQL_SRVC_VALUE_EXPRESSION),
    GI_CONST_BITMAP(SQL_SS_ADDITIONS), GI_CONST_BITMAP(SQL_SS_DELETIONS),
    GI_CONST_BITMAP(SQL_SSF_CONVERT), GI_CONST_BITMAP(SQL_SSF_LOWER),
    GI_CONST_BITMAP(SQL_SSF_SUBSTRING), GI_CONST_BITMAP(SQL_SSF_TRANSLATE),
    GI_CONST_BITMAP(SQL_SSF_TRIM_BOTH), GI_CONST_BITMAP(SQL_SSF_TRIM_LEADING),
    GI_CONST_BITMAP(SQL_SSF_TRIM_TRAILING), GI_CONST_BITMAP(SQL_SSF_UPPER),
    GI_CONST_BITMAP(SQL_SS_UPDATES), GI_CONST_BITMAP(SQL_SU_DML_STATEMENTS),
    ...
    GI_CONST_BITMAP(SQL_SU_TABLE_DEFINITION), GI_CONST_BITMAP(SQL_SVE_CASE),
    GI_CONST_BITMAP(SQL_SVE_CAST), GI_CONST_BITMAP(SQL_SVE_COALESCE),
    GI_CONST_BITMAP(SQL_SVE_NULLIF), GI_CONST_BITMAP(SQL_TC_ALL),
    GI_CONST_BITMAP(SQL_TC_DDL_COMMIT), GI_CONST_BITMAP(SQL_TC_DDL_IGNORE),
    GI_CONST_BITMAP(SQL_TC_DML), GI_CONST_BITMAP(SQL_TC_NONE),
...
#endif
    GI_CONST_BITMAP(SQL_US_UNION), GI_CONST_BITMAP(SQL_US_UNION_ALL),
    GI_CONST_BITMAP(SQL_U_UNION), GI_CONST_BITMAP(SQL_U_UNION_ALL),

    /* end of table */
    GI_CONST_BITMAP_END};

static VALUE dbc_getinfo(int argc, VALUE* argv, VALUE self)
{
    DBC* p = get_dbc(self);
    ...
    switch (info_found)
    {
        case 0:
            rb_raise(
                Cerror, "%s",
                set_err("Invalid info type for ODBC::Connection.get_info", 0));
            return Qnil;
        case 1:
            sprintf(buffer,
    ...
    switch (maptype)
    {
        case SQL_C_SHORT:
            ...
            sbuffer = 0;
            ret = SQLGetInfo(p->hdbc, (SQLUSMALLINT)info, (SQLPOINTER)&sbuffer,
                             len_in, &len_out);
            break;
        case SQL_C_LONG:
            len_in = sizeof(lbuffer);
            lbuffer = 0;
            ret = SQLGetInfo(p->hdbc, (SQLUSMALLINT)info, (SQLPOINTER)&lbuffer,
                             len_in, &len_out);
            break;
        default:
        case SQL_C_CHAR:
            ...
            ret = SQLGetInfo(p->hdbc, (SQLUSMALLINT)info, (SQLPOINTER)buffer,
                             len_in, &len_out);
            break;
    }
    if (!SQL_SUCCEEDED(ret))
...
static COLTYPE* make_coltypes(SQLHSTMT hstmt, int ncols, char** msgp)
{
    int i;
    COLTYPE* ret = NULL;
    ...
    for (i = 0; i < ncols; i++)
    {
        ...
        if (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                       SQLColAttributes(hstmt, ic, SQL_COLUMN_TYPE, NULL, 0,
                                        NULL, &type),
        ...
        if (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                       SQLColAttributes(hstmt, ic, SQL_COLUMN_DISPLAY_SIZE,
                                        NULL, 0, NULL, &size),
    ...
    for (i = 0; i < ncols; i++)
    {
        ...
        callsql(
            ...
            SQLColAttributes(hstmt, ic, SQL_COLUMN_TYPE, NULL, 0, NULL, &type),
            "SQLColAttributes(SQL_COLUMN_TYPE)");
        callsql(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                SQLColAttributes(hstmt, ic, SQL_COLUMN_DISPLAY_SIZE, NULL, 0,
                                 NULL, &size),
...
static PARAMINFO* make_paraminfo(SQLHSTMT hstmt, int nump, char** msgp)
{
    int i;
    ...
    for (i = 0; i < nump; i++)
    {
        ...
        if (!succeeded(
                ...
                SQLDescribeParam(hstmt, (SQLUSMALLINT)(i + 1),
                                 &paraminfo[i].type, &paraminfo[i].coldef,
                                 &paraminfo[i].scale, &paraminfo[i].nullable),
...
static void retain_paraminfo_override(STMT* q, int nump, PARAMINFO* paraminfo)
{
...
static VALUE wrap_stmt(VALUE dbc, DBC* p, SQLHSTMT hstmt, STMT** qp)
{
    VALUE stmt = Qnil;
    STMT* q;
    int i;
...
static VALUE make_result(VALUE dbc, SQLHSTMT hstmt, VALUE result, int mode)
{
    DBC* p;
    STMT* q;
    ...
    if ((mode & MAKERES_PREPARE) || (hstmt == SQL_NULL_HSTMT) ||
        (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
    ...
    return result;
error:
    callsql(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt, SQLFreeStmt(hstmt, SQL_DROP),
...
static VALUE make_column(SQLHSTMT hstmt, int i, int upc, int use_scn)
{
    VALUE obj, v;
    SQLUSMALLINT ic = i + 1;
    SQLLEN iv;
    ...
    if (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                   SQLColAttributes(
                       hstmt, ic, use_scn ? SQL_COLUMN_NAME : SQL_COLUMN_LABEL,
                       name, (SQLSMALLINT)sizeof(name), &name_len, NULL),
                   &msg,
    ...
    if (succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                  SQLColAttributes(hstmt, ic, SQL_COLUMN_TABLE_NAME, name,
                                   (SQLSMALLINT)sizeof(name), &name_len, NULL),
    ...
    if (succeeded(
            SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
            SQLColAttributes(hstmt, ic, SQL_COLUMN_TYPE, NULL, 0, NULL, &iv),
    ...
    iv = SQL_NO_NULLS;
    if (succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                  SQLColAttributes(hstmt, ic, SQL_COLUMN_NULLABLE, NULL, 0,
                                   NULL, &iv),
    ...
    if (succeeded(
            SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
            SQLColAttributes(hstmt, ic, SQL_COLUMN_SCALE, NULL, 0, NULL, &iv),
    ...
    iv = 0;
    if (succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                  SQLColAttributes(hstmt, ic, SQL_COLUMN_PRECISION, NULL, 0,
                                   NULL, &iv),
    ...
    iv = SQL_UNSEARCHABLE;
    if (succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                  SQLColAttributes(hstmt, ic, SQL_COLUMN_SEARCHABLE, NULL, 0,
                                   NULL, &iv),
    ...
    iv = SQL_FALSE;
    if (succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, hstmt,
                  SQLColAttributes(hstmt, ic, SQL_COLUMN_UNSIGNED, NULL, 0,
                                   NULL, &iv),
...
static VALUE make_param(STMT* q, int i)
{
    VALUE obj;
    int v;
...
static VALUE dbc_info(int argc, VALUE* argv, VALUE self, int mode)
{
    DBC* p = get_dbc(self);
    ...
    if (swhich != NULL)
    {
        ...
        val = rb_funcall(val, IDencode, 1, rb_encv);
#endif
        swhich = uc_from_utf((unsigned char*)StringValueCStr(val), -1);
        if (swhich == NULL)
        {
            rb_raise(Cerror, "%s", set_err("Out of memory", 0));
        }
    ...
    if (swhich2 != NULL)
    {
        ...
        val = rb_funcall(val, IDencode, 1, rb_encv);
#endif
        swhich2 = uc_from_utf((unsigned char*)StringValueCStr(val), -1);
        if (swhich2 == NULL)
        {
            uc_free(swhich);
            rb_raise(Cerror, "%s", set_err("Out of memory", 0));
...
static VALUE dbc_tables(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_TABLES);
}

static VALUE dbc_columns(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_COLUMNS);
}

static VALUE dbc_primkeys(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_PRIMKEYS);
}

static VALUE dbc_indexes(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_INDEXES);
}

static VALUE dbc_types(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_TYPES);
}

static VALUE dbc_forkeys(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_FORKEYS);
}

static VALUE dbc_tpriv(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_TPRIV);
}

static VALUE dbc_procs(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_PROCS);
}

static VALUE dbc_proccols(int argc, VALUE* argv, VALUE self)
{
    return dbc_info(argc, argv, self, INFO_PROCCOLS);
}

static VALUE dbc_speccols(int argc, VALUE* argv, VALUE self)
{
...
static VALUE dbc_commit(VALUE self) { return dbc_trans(self, SQL_COMMIT); }

static VALUE dbc_rollback(VALUE self) { return dbc_trans(self, SQL_ROLLBACK); }

static VALUE dbc_nop(VALUE self) { return Qnil; }
...
static VALUE dbc_transaction(VALUE self)
{
    ...
    if ((ret = rb_rescue2(dbc_transbody, a, dbc_transfail, a, rb_eException,
                          (VALUE)0)) != Qundef)
    {
        dbc_commit(self);
        return ret;
...
#if (ODBCVER >= 0x0300)
static VALUE do_attr(int argc, VALUE* argv, VALUE self, int op)
{
    SQLHENV henv = SQL_NULL_HENV;
    VALUE val;
    SQLLEN v = 0;
    ...
    if (val == Qnil)
    {
        if (!succeeded(henv, SQL_NULL_HDBC, SQL_NULL_HSTMT,
                       SQLGetEnvAttr(henv, (SQLINTEGER)op, (SQLPOINTER)&v,
                                     sizeof(v), &l),
...
static VALUE env_cpooling(int argc, VALUE* argv, VALUE self)
{
#if (ODBCVER >= 0x0300)
...
static VALUE env_cpmatch(int argc, VALUE* argv, VALUE self)
{
#if (ODBCVER >= 0x0300)
...
static VALUE env_odbcver(int argc, VALUE* argv, VALUE self)
{
#if (ODBCVER >= 0x0300)
...
#define OPT_CONST_INT(x, level)                                                \
    {                                                                          \
#x, x, level                                                           \
    }
#define OPT_CONST_END                                                          \
    {                                                                          \
        NULL, -1                                                               \
    }
static struct
...
static VALUE do_option(int argc, VALUE* argv, VALUE self, int isstmt, int op)
{
    DBC* p = NULL;
    STMT* q = NULL;
    VALUE val, val2 = Qnil;
    ...
    {
        if (!succeeded(
                SQL_NULL_HENV, p->hdbc, SQL_NULL_HSTMT,
                SQLSetConnectOption(p->hdbc, (SQLUSMALLINT)op, (SQLUINTEGER)v),
    ...
    {
        if (!succeeded(
                SQL_NULL_HENV, SQL_NULL_HDBC, q->hstmt,
                SQLSetStmtOption(q->hstmt, (SQLUSMALLINT)op, (SQLUINTEGER)v),
...
static VALUE dbc_autocommit(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_AUTOCOMMIT);
}

static VALUE dbc_concurrency(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_CONCURRENCY);
}

static VALUE dbc_maxrows(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_MAX_ROWS);
}

static VALUE dbc_timeout(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_QUERY_TIMEOUT);
}

static VALUE dbc_login_timeout(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_LOGIN_TIMEOUT);
}

static VALUE dbc_maxlength(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_MAX_LENGTH);
}

static VALUE dbc_rowsetsize(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_ROWSET_SIZE);
}

static VALUE dbc_cursortype(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_CURSOR_TYPE);
}

static VALUE dbc_noscan(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, SQL_NOSCAN);
}

static VALUE dbc_getsetoption(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 0, -1);
}

static VALUE stmt_concurrency(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_CONCURRENCY);
}

static VALUE stmt_maxrows(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_MAX_ROWS);
}

static VALUE stmt_timeout(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_QUERY_TIMEOUT);
}

static VALUE stmt_maxlength(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_MAX_LENGTH);
}

static VALUE stmt_rowsetsize(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_ROWSET_SIZE);
}

static VALUE stmt_cursortype(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_CURSOR_TYPE);
}

static VALUE stmt_noscan(int argc, VALUE* argv, VALUE self)
{
    return do_option(argc, argv, self, 1, SQL_NOSCAN);
}

static VALUE stmt_getsetoption(int argc, VALUE* argv, VALUE self)
{
...
static int scan_dtts(VALUE str, int do_d, int do_t, TIMESTAMP_STRUCT* ts)
{
    int yy = 0, mm = 0, dd = 0, hh = 0, mmm = 0, ss = 0, ff = 0, i;
    char c, *cstr = StringValueCStr(str);

    memset(ts, 0, sizeof(TIMESTAMP_STRUCT));
    if (((sscanf(cstr, "{ts '%d-%d-%d %d:%d:%d.%d' %c", &yy, &mm, &dd, &hh,
                 &mmm, &ss, &ff, &c) == 8) ||
         (sscanf(cstr, "{ts '%d-%d-%d %d:%d:%d' %c", &yy, &mm, &dd, &hh, &mmm,
                 &ss, &c) == 7)) &&
        (c == '}'))
    {
        ts->year = yy;
    ...
    if (do_d && (sscanf(cstr, "{d '%d-%d-%d' %c", &yy, &mm, &dd, &c) == 4) &&
        (c == '}'))
    {
        ts->year = yy;
        ts->month = mm;
    ...
    if (do_t && (sscanf(cstr, "{t '%d:%d:%d' %c", &hh, &mmm, &ss, &c) == 4) &&
        (c == '}'))
    {
        ts->hour = yy;
        ts->minute = mmm;
    ...
    ff = ss = 0;
    i = sscanf(cstr, "%d-%d-%d %d:%d:%d%c%d", &yy, &mm, &dd, &hh, &mmm, &ss, &c,
               &ff);
    if (i >= 5)
    {
...
static VALUE date_load(VALUE self, VALUE str)
{
    return date_load1(self, str, 1);
}

static VALUE date_init(int argc, VALUE* argv, VALUE self)
{
    DATE_STRUCT* date;
    VALUE d, m, y;
...
static VALUE date_dump(VALUE self, VALUE depth) { return date_to_s(self); }
...
static VALUE date_year(int argc, VALUE* argv, VALUE self)
{
    DATE_STRUCT* date;
    VALUE v;
...
static VALUE date_month(int argc, VALUE* argv, VALUE self)
{
    DATE_STRUCT* date;
    VALUE v;
...
static VALUE date_day(int argc, VALUE* argv, VALUE self)
{
    DATE_STRUCT* date;
    VALUE v;
...
static VALUE date_cmp(VALUE self, VALUE date)
{
    DATE_STRUCT *date1, *date2;
...
static VALUE time_load(VALUE self, VALUE str)
{
    return time_load1(self, str, 1);
}

static VALUE time_init(int argc, VALUE* argv, VALUE self)
{
    TIME_STRUCT* time;
    VALUE h, m, s;
...
static VALUE time_dump(VALUE self, VALUE depth) { return time_to_s(self); }
...
static VALUE time_hour(int argc, VALUE* argv, VALUE self)
{
    TIME_STRUCT* time;
    VALUE v;
...
static VALUE time_min(int argc, VALUE* argv, VALUE self)
{
    TIME_STRUCT* time;
    VALUE v;
...
static VALUE time_sec(int argc, VALUE* argv, VALUE self)
{
    TIME_STRUCT* time;
    VALUE v;
...
static VALUE time_cmp(VALUE self, VALUE time)
{
    TIME_STRUCT *time1, *time2;
...
static VALUE timestamp_load(VALUE self, VALUE str)
{
    return timestamp_load1(self, str, 1);
}

static VALUE timestamp_init(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
...
static VALUE timestamp_to_s(VALUE self)
{
    ...
    Data_Get_Struct(self, TIMESTAMP_STRUCT, ts);
    sprintf(buf, "%04d-%02d-%02d %02d:%02d:%02d %u", ts->year, ts->month,
            ts->day, ts->hour, ts->minute, ts->second,
            (unsigned int)ts->fraction);
    return rb_str_new2(buf);
}

static VALUE timestamp_dump(VALUE self, VALUE depth)
{
...
static VALUE timestamp_year(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_month(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_day(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_hour(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_min(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_sec(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_fraction(int argc, VALUE* argv, VALUE self)
{
    TIMESTAMP_STRUCT* ts;
    VALUE v;
...
static VALUE timestamp_cmp(VALUE self, VALUE timestamp)
{
    TIMESTAMP_STRUCT *ts1, *ts2;
...
static int param_num_check(STMT* q, VALUE pnum, int mkparaminfo, int needout)
{
    int vnum;
...
static VALUE stmt_param_type(int argc, VALUE* argv, VALUE self)
{
...
static VALUE stmt_param_iotype(int argc, VALUE* argv, VALUE self)
{
    VALUE pnum, piotype;
    int vnum, viotype;
    STMT* q;
...
static VALUE stmt_param_output_value(int argc, VALUE* argv, VALUE self)
{
    VALUE pnum, v;
    int vnum;
    STMT* q;
    ...
    switch (q->paraminfo[vnum].ctype)
    {
        ...
        case SQL_C_DATE:
        {
            ...
            if (q->dbcp != NULL && q->dbcp->rbtime == Qtrue)
            {
                ...
                sprintf(buffer, "%d-%d-%dT00:00:00%s", date->year, date->month,
                        date->day, p);
        ...
        case SQL_C_TIMESTAMP:
        {
            ...
            if (q->dbcp != NULL && q->dbcp->rbtime == Qtrue)
            {
                ...
                v = rb_funcall(rb_cTime,
                               ...
                               INT2NUM(ts->year), INT2NUM(ts->month),
                               INT2NUM(ts->day), INT2NUM(ts->hour),
                               INT2NUM(ts->minute), INT2NUM(ts->second), frac);
            }
            else
            {
                v = Data_Make_Struct(Ctimestamp, TIMESTAMP_STRUCT, 0, xfree,
                                     ts);
                *ts = *((TIMESTAMP_STRUCT*)q->paraminfo[vnum].outbuf);
            }
        }
...
#endif
        case SQL_C_CHAR:
            v = rb_str_new(q->paraminfo[vnum].outbuf, q->paraminfo[vnum].rlen);
            break;
    }
    return v;
}

static VALUE stmt_param_output_size(int argc, VALUE* argv, VALUE self)
{
    VALUE pnum, psize;
    int vnum, vsize;
    STMT* q;
...
static VALUE stmt_param_output_type(int argc, VALUE* argv, VALUE self)
{
    VALUE pnum, ptype;
    int vnum, vtype;
    STMT* q;
...
static VALUE stmt_cursorname(int argc, VALUE* argv, VALUE self)
{
    VALUE cn = Qnil;
    STMT* q;
#ifdef UNICODE
...
#endif
    if (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, q->hstmt,
                   SQLSetCursorName(q->hstmt, cp, SQL_NTS), &msg,
                   "SQLSetCursorName"))
    {
#ifdef UNICODE
        uc_free(cp);
...
static VALUE stmt_column(int argc, VALUE* argv, VALUE self)
{
    STMT* q;
    VALUE col;
    int use_scn = 0;
...
static VALUE stmt_columns(int argc, VALUE* argv, VALUE self)
{
    STMT* q;
    int i, use_scn = 0;
...
static VALUE stmt_param(int argc, VALUE* argv, VALUE self)
{
    STMT* q;
    VALUE par;
    int i;
...
static VALUE do_fetch(STMT* q, int mode)
{
            ...
            if (q->colnames == NULL)
            {
                ...
                for (i = 0; i < q->ncols; i++)
                {
                    ...
                    if (!succeeded(
                            ...
                            &msg, "SQLColAttributes(SQL_COLUMN_TABLE_NAME)"))
                    {
                        rb_raise(Cerror, "%s", msg);
                    }
                    if (name_len >= (SQLSMALLINT)sizeof(name))
                    ...
                    if (!succeeded(
                            SQL_NULL_HENV, SQL_NULL_HDBC, q->hstmt,
                            SQLColAttributes(
                                q->hstmt, (SQLUSMALLINT)(i + 1),
                                use_scn ? SQL_COLUMN_NAME : SQL_COLUMN_LABEL,
                                name, sizeof(name), &name_len, NULL),
                            &msg,
                            use_scn ? "SQLColAttributes(SQL_COLUMN_NAME)"
                                    : "SQLColAttributes(SQL_COLUMN_LABEL)"))
                    {
    ...
    for (i = 0; i < q->ncols; i++)
    {
        ...
        if (curlen == SQL_NO_TOTAL)
        {
            ...
            while ((curlen == SQL_NO_TOTAL) || (curlen > chunksize))
            {
                ...
                rc = SQLGetData(q->hstmt, (SQLUSMALLINT)(i + 1), type,
                ...
                ret = succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, q->hstmt, rc,
                                &msg, "SQLGetData");
                if (!ret)
                {
                    xfree(valp);
            ...
            switch (type)
            {
                ...
                case SQL_C_DATE:
                {
                    ...
                    if (q->dbcp != NULL && q->dbcp->rbtime == Qtrue)
                    {
                        ...
                        p = (q->dbcp->gmtime == Qtrue) ? "+00:00" : "";
                        sprintf(buffer, "%d-%d-%dT00:00:00%s", date->year,
                ...
                case SQL_C_TIMESTAMP:
                {
                    ...
                    {
                        v = Data_Make_Struct(Ctimestamp, TIMESTAMP_STRUCT, 0,
                                             xfree, ts);
                        *ts = *(TIMESTAMP_STRUCT*)valp;
                    }
                }
...
static VALUE stmt_fetch1(VALUE self, int bang)
{
    STMT* q;
    SQLRETURN ret;
    const char* msg;
    ...
    if ((err != NULL) &&
        ((strncmp(err, "IM001", 5) == 0) || (strncmp(err, "HYC00", 5) == 0)))
    {
    usef:
...
static VALUE stmt_fetch_first1(VALUE self, int bang, int nopos)
{
    STMT* q;
    SQLRETURN ret;
    const char* msg;
...
static VALUE stmt_fetch_scroll1(int argc, VALUE* argv, VALUE self, int bang)
{
    STMT* q;
    VALUE dir, offs;
    SQLRETURN ret;
...
static VALUE stmt_fetch_scroll(int argc, VALUE* argv, VALUE self)
{
    return stmt_fetch_scroll1(argc, argv, self, 0);
}

static VALUE stmt_fetch_scroll_bang(int argc, VALUE* argv, VALUE self)
{
    return stmt_fetch_scroll1(argc, argv, self, 1);
}

static VALUE stmt_fetch_many(VALUE self, VALUE arg)
{
...
static VALUE stmt_fetch_all(VALUE self) { return stmt_fetch_many(self, Qnil); }

static int stmt_hash_mode(int argc, VALUE* argv, VALUE self)
{
        ...
        (rb_obj_is_kind_of(withtab, rb_cHash) == Qtrue))
    {
        ...
        if (mode != DOFETCH_HASHN)
        {
            ...
            if (RTEST(v))
            {
                mode = (mode == DOFETCH_HASHK) ? DOFETCH_HASHK2 : DOFETCH_HASH2;
            }
        }
        return mode;
    }
...
static VALUE stmt_fetch_hash1(int argc, VALUE* argv, VALUE self, int bang)
{
    STMT* q;
    SQLRETURN ret;
...
static VALUE stmt_fetch_hash(int argc, VALUE* argv, VALUE self)
{
    if (rb_block_given_p())
    {
...
static VALUE stmt_fetch_hash_bang(int argc, VALUE* argv, VALUE self)
{
    if (rb_block_given_p())
    {
        return stmt_each_hash(argc, argv, self);
...
static VALUE stmt_fetch_first_hash1(int argc, VALUE* argv, VALUE self, int bang,
                                    int nopos)
{
    STMT* q;
    SQLRETURN ret;
...
static VALUE stmt_fetch_first_hash(int argc, VALUE* argv, VALUE self)
{
    ...
    if (rb_obj_is_kind_of(self, Cstmt) == Qtrue)
    {
        ...
        if (q->hstmt == SQL_NULL_HSTMT)
        {
            if (!succeeded(SQL_NULL_HENV, p->hdbc, q->hstmt,
                           SQLAllocStmt(p->hdbc, &q->hstmt), &msg,
                           "SQLAllocStmt"))
            {
                rb_raise(Cerror, "%s", msg);
            }
                            ...
                            SQLFreeStmt(q->hstmt, SQL_CLOSE), &msg,
...
static VALUE stmt_prep(int argc, VALUE* argv, VALUE self)
{
    return stmt_prep_int(argc, argv, self, MAKERES_BLOCK);
}

static int bind_one_param(int pnum, VALUE arg, STMT* q, char** msgp, int* outpp)
{
    ...
    if (coldef == 0)
    {
        switch (ctype)
        {
            ...
            default:
                 ...
                 * per patch from Paul G. Weiss, who was experiencing
                 * re-preparing of queries when the size of the bound string's
                 * were increasing for example select * from tabtest where name
                 * = ? then executing with 'paul' and then 'thomas' would cause
                 * SQLServer to prepare the query twice, but if we ran 'thomas'
                 * then 'paul', it would not re-prepare the query.  The key
                 * seems to be allocating enough space for the largest
                 * parameter. TBD: the default for this should be a tunable
                 * parameter.
                 */
                if ((stype == SQL_VARCHAR) &&
...
retry:
    ...
    if (!succeeded(
            ...
            SQLBindParameter(q->hstmt, (SQLUSMALLINT)(pnum + 1),
                             q->paraminfo[pnum].iotype, ctype, stype, coldef,
                             q->paraminfo[pnum].scale,
                             (ctype == SQL_C_WCHAR) ? *(SQLWCHAR**)valp : valp,
                             vlen, &q->paraminfo[pnum].rlen),
            msgp, "SQLBindParameter(%d)", pnum + 1))
#else
    if (!succeeded(SQL_NULL_HENV, SQL_NULL_HDBC, q->hstmt,
                   SQLBindParameter(q->hstmt, (SQLUSMALLINT)(pnum + 1),
                                    q->paraminfo[pnum].iotype, ctype, stype,
                                    coldef, q->paraminfo[pnum].scale, valp,
                                    vlen, &q->paraminfo[pnum].rlen),
                   msgp, "SQLBindParameter(%d)", pnum + 1))
#endif
    {
...
static VALUE stmt_exec_int(int argc, VALUE* argv, VALUE self, int mode)
{
    STMT* q;
...
static VALUE stmt_run(int argc, VALUE* argv, VALUE self)
{
    if (argc < 1)
    {
        rb_raise(rb_eArgError, "wrong # of arguments");
    ...
    if (argc == 1)
    {
        return stmt_prep_int(1, argv, self, MAKERES_EXECD | MAKERES_BLOCK);
    }
    return stmt_exec(argc - 1, argv + 1, stmt_prep_int(1, argv, self, 0));
}

static VALUE stmt_do(int argc, VALUE* argv, VALUE self)
{
    VALUE stmt;
...
static VALUE stmt_ignorecase(int argc, VALUE* argv, VALUE self)
{
    VALUE onoff = Qnil;
    int* flag = NULL;
...
static VALUE stmt_proc_init(int argc, VALUE* argv, VALUE self)
{
...
static VALUE stmt_proc_call(int argc, VALUE* argv, VALUE self)
{
    VALUE stmt, val;
...
static VALUE stmt_proc(int argc, VALUE* argv, VALUE self)
{
...
static VALUE stmt_procwrap(int argc, VALUE* argv, VALUE self)
{
...
static VALUE mod_dbcdisc(VALUE dbc) { return dbc_disconnect(0, NULL, dbc); }

static VALUE mod_connect(int argc, VALUE* argv, VALUE self)
{
...
static VALUE mod_2time(int argc, VALUE* argv, VALUE self)
{
    VALUE a1, a2;
...
static VALUE mod_2date(VALUE self, VALUE arg)
{
    VALUE y, m, d;
    int once = 0;
...
again:
    ...
    if ((!once) && (((m = date_load1(Cdate, arg, -1)) != Qnil) ||
...
static VALUE mod_trace(int argc, VALUE* argv, VALUE self)
{
    VALUE v = Qnil;
...
static VALUE mod_gc_threshold(int argc, VALUE* argv, VALUE self)
{
    VALUE v = Qnil;
...
static struct
{
    const char* name;
    int value;
} o_const[] = {O_CONST(SQL_CURSOR_FORWARD_ONLY),
               O_CONST(SQL_CURSOR_KEYSET_DRIVEN), O_CONST(SQL_CURSOR_DYNAMIC),
               O_CONST(SQL_CURSOR_STATIC), O_CONST(SQL_CONCUR_READ_ONLY),
               O_CONST(SQL_CONCUR_LOCK), O_CONST(SQL_CONCUR_ROWVER),
               O_CONST(SQL_CONCUR_VALUES), O_CONST(SQL_FETCH_NEXT),
               O_CONST(SQL_FETCH_FIRST), O_CONST(SQL_FETCH_LAST),
               O_CONST(SQL_FETCH_PRIOR), O_CONST(SQL_FETCH_ABSOLUTE),
               O_CONST(SQL_FETCH_RELATIVE), O_CONST(SQL_UNKNOWN_TYPE),
               O_CONST(SQL_CHAR), O_CONST(SQL_NUMERIC), O_CONST(SQL_DECIMAL),
               O_CONST(SQL_INTEGER), O_CONST(SQL_SMALLINT), O_CONST(SQL_FLOAT),
               O_CONST(SQL_REAL), O_CONST(SQL_DOUBLE), O_CONST(SQL_VARCHAR),
#ifdef SQL_DATETIME
               ...
               O_CONST(SQL_OV_ODBC2), O_CONST(SQL_OV_ODBC3),
#else
               O_CONST2(SQL_OV_ODBC2, 2), O_CONST2(SQL_OV_ODBC3, 3),
#endif
...
#endif
#else
               O_CONST2(SQL_CP_OFF, 0), O_CONST2(SQL_CP_ONE_PER_DRIVER, 0),
               O_CONST2(SQL_CP_ONE_PER_HENV, 0), O_CONST2(SQL_CP_DEFAULT, 0),
#endif
#ifdef SQL_ATTR_CP_MATCH
               O_CONST(SQL_CP_STRICT_MATCH), O_CONST(SQL_CP_RELAXED_MATCH),
...
#endif
               O_CONST(SQL_PARAM_TYPE_UNKNOWN), O_CONST(SQL_PARAM_INPUT),
               O_CONST(SQL_PARAM_OUTPUT), O_CONST(SQL_PARAM_INPUT_OUTPUT),
               O_CONST(SQL_DEFAULT_PARAM), O_CONST(SQL_RETURN_VALUE),
               O_CONST(SQL_RESULT_COL), O_CONST(SQL_PT_UNKNOWN),
               O_CONST(SQL_PT_PROCEDURE), O_CONST(SQL_PT_FUNCTION),
